# Changelog since v0.4.0
- Merge pull request #28 from casse-boubou/renovate/monero-project-monero-0.x

✨ Update MoneroD to v0.18.4.2 
- Merge branch 'main' into renovate/monero-project-monero-0.x 
- ⬆️ Update Addon Base Image to v8 and dependency 
- ✨ Update MoneroD to v0.18.4.2 
- Merge pull request #26 from casse-boubou/renovate/monero-project-monero-0.x

✨ Update MoneroD to v0.18.4.0 
- ✨ Update MoneroD to v0.18.4.0 
- Merge pull request #25 from casse-boubou/renovate/ghcr.io-hassio-addons-debian-base-7.x

⬆️ Update Addon Base Image to v7.8.1 
- ⬆️ Update Addon Base Image to v7.8.1 
- Create .gitignore 
- Merge pull request #23 from casse-boubou/renovate/ghcr.io-hassio-addons-debian-base-7.x

⬆️ Update Addon Base Image to v7.7.1 
- ⬆️ Update Addon Base Image to v7.7.1 
- Merge pull request #24 from casse-boubou/renovate/migrate-config

Migrate renovate config 
- Prettified Code! 
- Migrate config .github/renovate.json 
- Merge pull request #22 from casse-boubou/renovate/ghcr.io-hassio-addons-debian-base-7.x

⬆️ Update Addon Base Image to v7.7.0 
- ⬆️ Update Addon Base Image to v7.7.0 
